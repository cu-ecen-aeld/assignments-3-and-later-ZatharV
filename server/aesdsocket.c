 ...
 * @file      aesdsocket.c
 * @author    Atharv More
 * @brief     Multi-threaded socket server with aesdchar device support.
 *            Assignment 9: Added AESDCHAR_IOCSEEKTO:X,Y socket command.
 */
...
/* ioctl definitions (userspace mirror of aesd_ioctl.h) */
struct aesd_seekto {
    uint32_t write_cmd;
...
void *timestamp_handler(void *thread_param)
{
    while (keep_running) {
        int i;
        for (i = 0; i < 10 && keep_running; i++) sleep(1);
        ...
        len = snprintf(final_str, sizeof(final_str), "timestamp:%s\n", time_buf);
        pthread_mutex_lock(&file_mutex);
        int fd = open(DATA_FILE, O_WRONLY | O_CREAT | O_APPEND, 0644);
        if (fd != -1) { write(fd, final_str, len); close(fd); }
...
void *thread_handler(void *thread_param)
{
    ...
    if (total_received == 0) goto cleanup;

    pthread_mutex_lock(&file_mutex);

#if USE_AESD_CHAR_DEVICE
    if (strncmp(recv_buffer, SEEKTO_CMD_PREFIX, strlen(SEEKTO_CMD_PREFIX)) == 0) {
        /*
         * Assignment 9: AESDCHAR_IOCSEEKTO:X,Y command
         * Parse X,Y, send ioctl, read back using SAME fd to preserve f_pos.
         * Do NOT write this string to the device.
         */
        const char *params = recv_buffer + strlen(SEEKTO_CMD_PREFIX);
        unsigned int x = 0, y = 0;
        if (sscanf(params, "%u,%u", &x, &y) != 2) {
            syslog(LOG_ERR, "Invalid IOCSEEKTO format");
            pthread_mutex_unlock(&file_mutex);
            goto cleanup;
        }

        /* ONE fd for both ioctl and read - f_pos must be preserved */
        ...
        if (ioctl(fd, AESDCHAR_IOCSEEKTO, &seekto) < 0) {
            syslog(LOG_ERR, "IOCSEEKTO failed: %s", strerror(errno));
            close(fd);
            pthread_mutex_unlock(&file_mutex);
            goto cleanup;
        }

        /* Read from ioctl-set position and send to client */
        ...
        /* Normal command: write to device, read all back */
        int wfd = open(DATA_FILE, O_WRONLY | O_APPEND);
        if (wfd == -1) {
            syslog(LOG_ERR, "open %s for write failed: %s", DATA_FILE, strerror(errno));
            pthread_mutex_unlock(&file_mutex);
            goto cleanup;
        }
...
int main(int argc, char *argv[])
{
    ...
    if (server_fd < 0) { syslog(LOG_ERR, "socket failed"); return -1; }
    ...
    while (keep_running) {
        ...
        if (!new_thread) { close(client_fd); continue; }
        ...
        if (pthread_create(&new_thread->thread_id, NULL, thread_handler, new_thread) != 0) {
            free(new_thread); close(client_fd);
        } else {
